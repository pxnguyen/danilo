...
save_file_name = sprintf('active_testing/res_sm_mcn_%s.mat', opts.search_mode);
rng(1);
res = struct();
...
while ~done
  ...
  new_vetted_set = mcn(prob, res.info(iter-1).to_use_vetted, vetted_labels, opts);
...
exp_name = sprintf('sm_mcn_%d', current_budget);
